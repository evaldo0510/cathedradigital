...
CREATE TABLE IF NOT EXISTS public.saints_enrichment_runs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  kind text NOT NULL DEFAULT 'heuristic',
  limit_n integer,
  processed integer NOT NULL DEFAULT 0,
  updated integer NOT NULL DEFAULT 0,
  country_hits integer NOT NULL DEFAULT 0,
  vocation_hits integer NOT NULL DEFAULT 0,
  remaining_missing_country integer NOT NULL DEFAULT 0,
  remaining_missing_vocation integer NOT NULL DEFAULT 0,
...
GRANT SELECT ON public.saints_enrichment_runs TO authenticated;
GRANT ALL ON public.saints_enrichment_runs TO service_role;

ALTER TABLE public.saints_enrichment_runs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins can view saints enrichment runs" ON public.saints_enrichment_runs;
CREATE POLICY "Admins can view saints enrichment runs"
  ON public.saints_enrichment_runs
  FOR SELECT
  TO authenticated
...
BEGIN
  IF auth.uid() IS NOT NULL AND NOT auth_internal.has_role(auth.uid(), 'admin'::app_role) THEN
...
REVOKE ALL ON FUNCTION public.run_saints_enrichment_heuristic(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.run_saints_enrichment_heuristic(integer) TO authenticated;

SELECT public.run_saints_enrichment_heuristic(NULL);
