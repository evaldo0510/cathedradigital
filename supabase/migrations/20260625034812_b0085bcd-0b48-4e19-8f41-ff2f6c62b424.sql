...
BEGIN
  ...
  WITH base AS (
    SELECT
      to_timestamp(floor(extract(epoch FROM e.created_at) / v_window_s) * v_window_s) AS bucket_start,
      e.abbrev,
      e.cache,
      e.l1_phase,
      e.total_ms,
  ...
  worst AS (
    SELECT
      b.bucket_start,
      b.abbrev,
      array_agg(b.correlation_id ORDER BY b.total_ms DESC NULLS LAST) FILTER (WHERE b.correlation_id IS NOT NULL) AS ids
    FROM (
      SELECT
        bucket_start, abbrev, correlation_id, total_ms,
        row_number() OVER (PARTITION BY bucket_start, abbrev ORDER BY total_ms DESC NULLS LAST) AS rn
      FROM base
    ) b
    WHERE b.rn <= 5
    GROUP BY b.bucket_start, b.abbrev
  )
  SELECT
    b.bucket_start,
    b.abbrev,
  ...
  FROM base b
  LEFT JOIN worst w ON w.bucket_start = b.bucket_start AND w.abbrev = b.abbrev
  GROUP BY b.bucket_start, b.abbrev, w.ids
  ORDER BY b.bucket_start ASC, b.abbrev ASC;
END;
$function$;
