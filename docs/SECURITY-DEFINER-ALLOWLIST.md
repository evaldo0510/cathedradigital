...
| 6 | `cleanup_bible_audit_action_logs` | `(p_triggered_by text, p_override_days integer)` | anon, authenticated, service_role | **Reavaliar em Sprint B.** Função de manutenção invocável via cron público. Exposição a `anon` é histórica; hoje o cron autentica com service-role. Marcada como dívida técnica de baixa severidade. |
