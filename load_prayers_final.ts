...
async function run() {
  ...
  for (const p of PRAYERS) {
    try {
      ...
      console.error(`Falha fatal [${p.slug}]:`, e);
      errors++;
    }
  }

  console.log('\nBanco: ' + (errors === 0 ? 'PASS' : 'PARTIAL_FAIL'));
  console.log('Registros inseridos/atualizados: ' + inserted);
  console.log('STATUS FINAL: ' + (errors === 0 ? 'CERTIFIED' : 'BLOCKED - INFRASTRUCTURE'));
}
