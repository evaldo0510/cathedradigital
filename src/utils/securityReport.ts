...
export const generateSecurityScanPDF = (scan: any, securityLogs: any[]) => {
  ...
  doc.text(`ID do Scan: ${scan.id}`, 14, 42);
  doc.text(`Status: ${scan.status.toUpperCase()}`, 14, 48);
  doc.text(`Score de Conformidade: ${scan.compliance_score}%`, 14, 54);
  doc.text(`Iniciado em: ${new Date(scan.started_at).toLocaleString()}`, 14, 60);

  // Issues Section
  doc.setFontSize(14);
  doc.setTextColor(233, 69, 96);
  doc.text('Issues Detectadas (Novas e Remanescentes):', 14, 75);
  
  const issues = scan.issues_found || [];
  if (issues.length === 0) {
    doc.setFontSize(11);
    doc.setTextColor(46, 204, 113);
    doc.text('Nenhuma issue de alta severidade encontrada.', 14, 85);
  } else {
    (doc as any).autoTable({
      startY: 80,
      head: [['Severidade', 'Categoria', 'Descrição']],
      body: issues.map((i: any) => [
        { content: (i.level || 'WARN').toUpperCase(), styles: { textColor: i.level === 'high' ? [233, 69, 96] : [100] } },
        i.category || 'RLS',
  ...
  doc.setFontSize(14);
  doc.setTextColor(26, 26, 46);
  doc.text('Trechos do Log de Alterações de Política:', 14, finalY);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Documento gerado automaticamente para fins de PR e Auditoria - Página ${i} de ${pageCount}`, 14, 285);
  }
