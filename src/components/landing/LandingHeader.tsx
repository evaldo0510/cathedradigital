  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-1000 ${
        isScrolled
          ? "py-lg bg-background/80 backdrop-blur-3xl border-b border-border/5"
          : "py-xl bg-transparent"
      }`}
    >
      <div className="app-container flex items-center justify-between">
        <Link 
          to={AppRoute.HOME}
          className="flex items-center gap-md cursor-pointer group focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 rounded-full p-2xs transition-shadow"
          aria-label="Cathedra - Página Inicial"
        >
          <Icons.Logo className="w-2xl h-2xl transition-all duration-1000 group-hover:scale-105" variant="gold" />
          <div className="hidden sm:flex flex-col">
            <h2 className="text-xl font-display font-medium text-primary tracking-[0.4em] uppercase leading-none">CATHEDRA</h2>
            <span className="text-[8px] font-black uppercase tracking-[0.6em] text-secondary/70 mt-2xs">Digital Sanctuarium</span>
          </div>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-lg">
          {navLinks.map((link) => (
            <Button
              ...
              className="h-auto py-2xs px-xs text-premium-small font-bold uppercase tracking-[0.3em] text-muted-foreground hover:text-primary transition-colors relative group shadow-none"
              type="button"
            >
              {link.name}
              <span className="absolute -bottom-2xs left-0 w-0 h-3xs bg-primary transition-all group-hover:w-full" />
            </Button>
          ))}
                ...
                >
                  {languages.map((l) => (
                    <Button
                      ...
                      className={`w-full px-md py-xs justify-between font-normal tracking-normal shadow-none ${lang === l.code ? 'text-primary font-bold' : 'text-muted-foreground'}`}
                      type="button"
                    >
                      <span>{l.label}</span>
                      <span>{l.flag}</span>
        ...
        <div className="flex items-center gap-md">
          <Button
            ...
            className="w-xl h-xl rounded-full text-muted-foreground hover:text-primary transition-colors focus-visible:ring-2 focus-visible:ring-primary"
          ...
          <HomeButton
            variant="ghost"
            size="sm"
            className={`hidden xs:flex rounded-full px-md sm:px-lg shadow-none transition-all ${isScrolled ? 'text-primary' : ''}`}
            onClick={() => navigate(AppRoute.LOGIN)}
          >
            Começar <ChevronRight className="w-md h-md ml-2xs" />
          </HomeButton>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            <div className="app-container py-xl flex flex-col gap-lg">
              {navLinks.map((link) => (
                <Button
                  key={link.name}
                  variant="ghost"
