...
const LandingHeader = () => {
  ...
  return (
    <header
      className={`fixed top-spacing-0 left-0 right-0 z-[100] transition-all duration-1000 ${
        isScrolled
      ...
      <div className="app-container flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-spacing-lg">
          {navLinks.map((link) => (
            <Button
              ...
              className="h-auto py-spacing-2xs px-spacing-xs text-premium-small font-bold uppercase tracking-[0.3em] text-muted-foreground hover:text-primary transition-colors relative group shadow-premium-none"
              type="button"
            >
              {link.name}
          ...
          <div className="relative">
            <Button 
              ...
              className="flex items-center gap-spacing-xs text-premium-xs font-bold uppercase tracking-[0.2em] text-muted-foreground hover:text-primary transition-colors shadow-premium-none"
            ...
            <AnimatePresence>
              {showLangMenu && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className="absolute right-0 mt-spacing-xs w-spacing-4xl bg-background border border-border rounded-premium-full shadow-premium-hover overflow-hidden"
                >
                  {languages.map((l) => (
                    <Button
                      key={l.code}
        ...
        <div className="flex items-center gap-spacing-md">
          ...
          <Button
            ...
            className="hidden sm:flex text-premium-small font-bold uppercase tracking-[0.3em] text-muted-foreground hover:text-primary transition-colors shadow-premium-none"
            ...
            <div className="app-container py-spacing-xl flex flex-col gap-spacing-lg">
              {navLinks.map((link) => (
                <Button
                  ...
                  variant="ghost"
                  onClick={() => handleNavClick(link.href)}
                  className="text-premium-lg font-serif font-bold text-left text-foreground hover:text-primary transition-colors outline-none focus:text-primary justify-start h-auto px-spacing-0"
                >
                  {link.name}
                </Button>
              ))}
