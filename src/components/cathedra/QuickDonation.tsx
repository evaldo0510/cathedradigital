          ...
          >
            <CardContent className="p-md pt-0 space-y-md">
              <div className="grid grid-cols-4 gap-xs" role="group" aria-label="Valores sugeridos para doação">
                {DONATION_PRESETS.map((val) => (
                  <Button
                    ...
                    className={`text-xs font-bold h-xl focus-visible:ring-2 focus-visible:ring-secondary outline-none ${amount === val ? 'bg-secondary text-secondary-foreground hover:bg-secondary/90' : 'border-secondary/30 text-secondary'}`}
                  >
                    R$ {val}
                  </Button>
                ))}
              ...
              <div className="flex gap-xs items-center">
                ...
                <div className="relative flex-1">
                  <span className="absolute left-sm top-2xs/2 -translate-y-1/2 text-xs font-bold text-muted-foreground">R$</span>
                  <Input
                    type="number"
                    min={1}
                    value={amount || ''}
                    ...
                    placeholder="0,00"
                    className="h-xl pl-xl text-xs font-bold bg-background border-secondary/10 rounded-full focus:ring-1 focus:ring-secondary/20"
                  />
                </div>
              </div>
            </CardContent>
            <CardFooter className="p-md pt-0">
              <Button
                ...
                className="btn-premium-secondary w-full h-2xl rounded-full text-xs font-black uppercase tracking-widest bg-secondary text-secondary-foreground hover:bg-secondary/90 shadow-premium shadow-secondary/20 transition-all"
              >
