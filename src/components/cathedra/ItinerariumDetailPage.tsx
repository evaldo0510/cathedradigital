...
const ItinerariumDetailPage: React.FC = () => {
  ...
  if (loading || !itinerarium) return <div className="p-4xl text-center">Carregando jornada...</div>;
    ...
    <div className="max-w-2xl mx-auto py-2xl md:py-4xl space-y-16">
      ...
      >
        <Button 
          ...
          className="group gap-xs text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 hover:text-primary transition-colors"
        >
          <ArrowLeft className="w-sm h-sm group-hover:-translate-x-0.5 transition-transform" /> Voltar para Trilhas
        </Button>
      </motion.div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-md flex-wrap">
          ...
          <Button
            variant="outline"
            size="sm"
            className="rounded-full gap-xs border-primary/10 text-[10px] font-black uppercase tracking-widest"
            onClick={exportFullPDF}
          >
            <FileDown className="w-md h-md" /> Exportar Reflexões
          </Button>
        </div>
      ...
      <Card className="premium-card bg-primary/[0.01] border-primary/5 rounded-[2.5rem] shadow-none overflow-hidden relative">
        ...
        <CardContent className="p-xl md:p-xl space-y-6 relative z-10">
          ...
          <Progress value={progress} className="h-2xs bg-primary/5" />
        </CardContent>
      </Card>
        ...
        {steps.map((step, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                <CardContent className="p-lg md:p-xl flex items-center justify-between gap-lg">
                  <div className="flex items-center gap-lg flex-1 min-w-0">
                    <div className={`w-2xl h-2xl rounded-premium flex-shrink-0 flex items-center justify-center transition-all duration-700 ${
                      isCompleted 
                        ? 'bg-primary text-primary-foreground shadow-premium' 
                        : 'bg-primary/5 text-primary/40 border border-primary/10'
                    }`}>
                      {isCompleted ? <Check className="w-md h-md" /> : <span className="text-sm font-bold">{idx + 1}</span>}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-xs mb-2xs">
                        ...
                        {!step.is_free && <Icons.Lock className="w-sm h-sm text-primary/60" />}
                      </div>
                      <div className="flex items-center gap-sm text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">
                        <span className="flex items-center gap-2xs"><Clock className="w-sm h-sm" /> {step.duration_minutes} MIN</span>
                        <div className="w-2xs h-2xs rounded-full bg-border" />
                  ...
                  {!isLocked && (
                    <Button 
                      ...
                      className="group/btn h-2xl px-lg rounded-full border border-primary/5 hover:bg-primary hover:text-primary-foreground transition-all duration-500 text-[10px] font-black uppercase tracking-widest"
                    >
                      {isCompleted ? 'Revisitar' : 'Iniciar'}
                      <ChevronRight className="w-md h-md ml-2xs group-hover/btn:translate-x-1 transition-transform" />
                    </Button>
                  )}
                </CardContent>
              </Card>
