...
const SantoProfile: React.FC<SantoProfileProps> = ({ saint, showHeader = false, hideAILearn = false }) => {
  ...
  return (
    <article className="space-y-spacing-2xl" aria-label={`Perfil de ${saint.name}`}>
      ...
      {/* ── Virtudes ───────────────────────────────────────────── */}
      {virtues.length > 0 && (
        <section aria-labelledby="virt-heading" className="space-y-spacing-md">
          <div className="flex items-center gap-spacing-xs text-primary">
            <Icons.Shield className="w-spacing-md h-spacing-md" aria-hidden="true" />
            <h3 id="virt-heading" className="text-premium-small font-black uppercase tracking-[0.2em]">Virtudes</h3>
          </div>
