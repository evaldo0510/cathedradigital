...
const CathedraCard = React.forwardRef<HTMLDivElement, CathedraCardProps>(
  ({ className, variant = 'default', padding = 'md', hover = false, children, ...props }, ref) => {
    ...
    const variantStyles = {
      ...
      glass: 'bg-background/20 backdrop-blur-xl border border-primary/[0.01] dark:border-primary/[0.02] rounded-premium shadow-none will-change-[transform,opacity]',
    };
    ...
    return (
      <motion.div
        ...
        whileHover={settings.reduceAnimations ? {} : { y: -2, scale: 1, transition: { duration: 0.4, ease: [0.16, 1, 0.3, 1] } }}
        {...props}
      >
        {children}
      </motion.div>
