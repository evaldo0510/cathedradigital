...
const ReadingCard: React.FC<{
  ...
  <motion.div
    ...
    className="space-y-6 premium-card p-xl group relative overflow-hidden"
  >
    ...
    <div className="flex items-center justify-between relative z-10">
      <div className="flex items-center gap-sm">
        <div className="p-xs rounded-premium bg-primary/5 text-primary group-hover:bg-primary group-hover:text-white transition-all shadow-soft">{icon}</div>
        <div>
          <h2 className="text-premium-tiny font-black uppercase tracking-[0.25em] text-primary">{label}</h2>
          <p className="text-premium-tiny font-bold text-secondary/60 uppercase tracking-[0.2em] mt-3xs">{reference}</p>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-wrap gap-sm pt-lg border-t border-border/40">
      ...
      <Button variant="secondary" size="sm" className="rounded-full ml-auto h-xl px-xl bg-secondary/10 border-none hover:bg-secondary/20 text-primary shadow-soft" onClick={onReflect}><Icons.Lectio className="w-md h-md mr-xs text-secondary" /> Lectio Divina</Button>
    </div>
  </motion.div>
);
          ...
          {activeTab === 'liturgia' && (
            <div {...getTabPanelProps('panel-liturgia', 'tab-liturgia', activeTab === 'liturgia', "max-w-2xl mx-auto space-y-10 animate-in fade-in duration-500 outline-none")}>
              <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 text-center">
                ...
                {isOfflineData && <div className="flex items-center justify-center gap-xs text-premium-tiny font-black uppercase tracking-widest text-muted-foreground bg-muted/50 rounded-premium px-md py-xs mt-md mx-auto w-fit"><Icons.WifiOff className="w-sm h-sm" /> <span>Modo Offline</span></div>}
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-primary text-white rounded-[2rem] p-xl text-center space-y-6 shadow-premium-hover">
              ...
              {saintsToday.length > 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="bg-muted/30 border border-border rounded-[2rem] p-xl flex flex-col items-center text-center space-y-4">
