...
export const PageHeaderSkeleton: React.FC = () => (
  <div className="text-center space-y-4 pt-8 mb-10 animate-pulse">
    ...
    <div className="h-10 w-64 mx-auto bg-muted rounded-full" />
    <div className="h-4 w-80 mx-auto bg-muted/60 rounded-full" />
  </div>
);
...
export const CardGridSkeleton: React.FC<{ count?: number }> = ({ count = 6 }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
    {Array.from({ length: count }).map((_, i) => (
      <Card key={i} className="h-56 rounded-full bg-muted/30 border border-border/40 shadow-sm" />
    ))}
  </div>
);
...
export const ListSkeleton: React.FC<{ count?: number }> = ({ count = 5 }) => (
  <div className="space-y-4 animate-pulse">
    {Array.from({ length: count }).map((_, i) => (
      <div key={i} className="h-20 rounded-full bg-muted/20 border border-border/30" />
    ))}
  </div>
);
...
export const SaintGridSkeleton: React.FC<{ count?: number }> = ({ count = 8 }) => (
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 animate-pulse">
    {Array.from({ length: count }).map((_, i) => (
      <div key={i} className="h-48 rounded-full bg-muted" />
    ))}
  </div>
);
