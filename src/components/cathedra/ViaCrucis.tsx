...
const ViaCrucis: React.FC = () => {
    ...
    return (
      <motion.div className="max-w-5xl mx-auto space-y-12 pb-2xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
        ...
        <div className="flex justify-center">
          <Button onClick={() => setIsJourney(true)} size="lg" className="h-3xl px-xl gap-sm rounded-full shadow-premium-hover">
            <Play className="w-md h-md fill-current" /> Iniciar Via Sacra
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-md">
          {STATIONS.map((s, i) => (
            <Button key={i} onClick={() => { setCurrentStation(i); setIsJourney(true); }}
              className="text-left p-lg rounded-full bg-card border border-border hover:border-primary/40 hover:shadow-premium-hover hover:-translate-y-1 transition-all group relative overflow-hidden">
              <div className="absolute top-0 right-0 p-lg opacity-[0.03] group-hover:opacity-[0.06] transition-opacity">
                <Cross className="w-4xl h-4xl -mr-xl -mt-xl rotate-12" />
              </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 pb-2xl animate-in fade-in duration-700">
      ...
      {/* Progress */}
      <div className="flex gap-2xs px-xs">
        {STATIONS.map((_, i) => (
          <div key={i} className={`flex-1 h-2xs rounded-full transition-all duration-500 ${i <= currentStation ? 'bg-primary shadow-[0_0_8px_rgba(var(--primary),0.3)]' : 'bg-border'}`} />
        ))}
      </div>
      ...
      <div className="flex gap-md justify-center px-md">
        ...
        {currentStation < STATIONS.length - 1 ? (
          ...
          >
            Próxima <ChevronRight className="w-md h-md" />
          </Button>
        ) : (
          <Button 
          ...
          >
            <Icons.Cross className="w-md h-md" /> Concluir
          </Button>
        )}
      </div>
    </div>
