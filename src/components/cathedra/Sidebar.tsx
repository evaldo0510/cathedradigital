...
const Sidebar = React.memo(React.forwardRef<HTMLElement, SidebarProps>(({ onClose, user, isDark, onToggleDark, isSpeaking, onToggleSpeak, onSignOut }, ref) => {
  ...
  const sections = [
    ...
    }] : []),
    {
      ...
      items: [
        ...
        { label: 'Centro de Comando', path: AppRoute.BUSCAR, icon: <Icons.Search className="w-5 h-5" /> },
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Glossário da Fé', path: AppRoute.GLOSSARY, icon: <Icons.Glossary className="w-5 h-5" /> },
  ...
  return (
    <>
      <aside ref={ref} className="h-full w-72 bg-background border-r border-primary/5 flex flex-col p-5 overflow-hidden">
