    ...
    return (
      <div className="max-w-spacing-4xl mx-auto space-y-spacing-lg">
        ...
        <div className="flex items-center gap-spacing-md">
          ...
          <img src={selectedApparition.imageSrc} alt={selectedApparition.title} className="w-spacing-3xl h-spacing-3xl rounded-premium-full object-cover shadow-premium" loading="lazy" />
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-spacing-sm">
          {[
            ...
            <div key={fact.label} className="p-spacing-sm rounded-premium bg-card border border-border">
              <p className="text-premium-xs font-black uppercase tracking-widest text-primary mb-spacing-2xs">{fact.label}</p>
              <p className="text-premium-xs font-semibold text-foreground">{fact.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-spacing-2xs p-spacing-2xs bg-muted rounded-premium">
          {[
            ...
            <Button
              ...
              className={`flex-1 flex items-center justify-center gap-spacing-xs py-spacing-xs rounded-premium-full text-premium-xs font-bold transition-all ${
                activeTab === tab.id ? 'bg-card text-foreground shadow-premium-md' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              {tab.icon}
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="bg-card border border-border rounded-premium-full p-spacing-lg md:p-spacing-xl"
          >
            {activeTab === 'historia' && (
              <div className="space-y-spacing-md">
                <h2 className="text-premium-lg font-serif font-bold text-foreground">A História da Aparição</h2>
                <p className="font-serif text-foreground/90 leading-[1.9] text-premium-base">
            ...
            {activeTab === 'mensagem' && (
              <div className="space-y-spacing-lg">
                <h2 className="text-premium-lg font-serif font-bold text-foreground">A Mensagem de Maria</h2>
                <blockquote className="border-l-4 border-primary pl-spacing-md py-spacing-xs">
                  <p className="font-serif italic text-foreground/90 leading-[1.9] text-premium-base">
    ...
    <div className="max-w-5xl mx-auto space-y-spacing-xl">
      ...
      <div className="text-center space-y-spacing-sm">
        <div className="inline-flex items-center gap-spacing-xs px-spacing-sm py-spacing-2xs bg-primary/10 rounded-premium">
          ...
          <span className="text-premium-xs font-black uppercase tracking-[0.2em] text-primary">Aparições Marianas</span>
        </div>
        <h1 className="text-premium-3xl md:text-premium-5xl font-serif font-bold text-foreground">Aparições de Nossa Senhora</h1>
        ...
        {APPARITIONS.map((a, i) => (
          <React.Fragment key={a.id}>
            <div className="flex flex-col items-center gap-spacing-2xs">
              <img src={a.imageSrc} alt={a.title} className="w-spacing-xl h-spacing-xl md:w-spacing-2xl md:h-spacing-2xl rounded-premium-full object-cover shadow-premium-md" loading="lazy" />
              <span className="text-premium-xs font-black text-primary">{a.year}</span>
            </div>
            ...
            >
              <div className="relative">
                ...
                <Button
                  ...
                  className={`absolute top-spacing-sm right-spacing-sm p-spacing-xs rounded-premium-full  transition-all ${isFav ? 'bg-primary/20 text-primary' : 'bg-black/30 text-white/80 hover:text-white'}`}
                  title={isFav ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}
                >
                  <Icons.Heart className={`w-spacing-md h-spacing-md ${isFav ? 'fill-primary' : ''}`} />
                </Button>
              ...
              <div className="p-spacing-md">
                <div className="mb-spacing-sm">
                  <span className="text-premium-xs font-black text-primary uppercase tracking-widest">{apparition.country} • {apparition.year}</span>
                  <h2 className="text-premium-lg md:text-premium-xl font-serif font-bold text-foreground mt-spacing-2xs group-hover:text-primary transition-colors">
                    {apparition.title}
                  </h2>
                </div>
                <p className="text-premium-sm text-muted-foreground mb-spacing-sm line-clamp-spacing-xs">{apparition.summary}</p>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-spacing-xs text-muted-foreground">
                    ...
                    <span className="text-premium-xs font-bold">{apparition.seer.split(',')[0]}</span>
                  </div>
      ...
      <div className="bg-card border border-border rounded-premium p-spacing-lg text-center space-y-spacing-sm">
        ...
        <p className="text-premium-sm text-muted-foreground max-w-spacing-xl mx-auto">
