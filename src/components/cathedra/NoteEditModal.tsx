...
export const NoteEditModal: React.FC<NoteEditModalProps> = ({ 
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-spacing-md sm:p-spacing-lg">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            <div className="space-y-spacing-md">
              <p className="text-[10px] font-bold uppercase tracking-widest text-primary/40 px-spacing-2xs">Cor do Destaque</p>
              <div className="flex gap-spacing-sm">
                {COLORS.map((c) => (
                  ...
                  >
                    {color === c.name && <Icons.Check className="w-spacing-md h-spacing-md text-primary" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-spacing-md">
               <p className="text-[10px] font-bold uppercase tracking-widest text-primary/40 px-spacing-2xs">Anotação</p>
               <textarea
                ref={textareaRef}
                value={text}
            ...
            <div className="flex flex-col sm:flex-row gap-spacing-md">
                ...
                <div className="flex-1 flex gap-spacing-xs">
                  {!showDeleteConfirm ? (
                    <Button 
                      ...
                      className="flex-1 h-spacing-2xl rounded-premium text-[11px] font-bold uppercase tracking-widest text-destructive hover:bg-destructive/5"
                    >
                      <Icons.Trash className="w-spacing-md h-spacing-md mr-spacing-xs" />
                      Excluir
                    </Button>
                  ) : (
                    <>
                      <Button 
                        ...
                        className="flex-1 h-spacing-2xl rounded-premium text-[11px] font-bold uppercase tracking-widest text-muted-foreground"
                      >
                        Não
                      </Button>
                      <Button 
                        variant="destructive" 
                        onClick={onDelete}
                        className="flex-1 h-spacing-2xl rounded-premium text-[11px] font-bold uppercase tracking-widest"
                      >
                        Sim
                      </Button>
                    </>
              ...
              {!showDeleteConfirm && (
                <div className="flex-[2] flex gap-spacing-md">
                  <Button 
                    variant="ghost" 
                    onClick={onClose}
                    className="flex-1 h-spacing-2xl rounded-premium text-[11px] font-bold uppercase tracking-widest text-muted-foreground hover:bg-muted"
                  >
                    Cancelar
                  </Button>
                  <Button 
                    onClick={() => onSave(text, color)}
                    className="flex-[1.5] h-spacing-2xl bg-primary text-primary-foreground rounded-premium px-spacing-xl text-[11px] font-bold uppercase tracking-widest shadow-premium hover:shadow-premium-hover transition-all"
                  >
