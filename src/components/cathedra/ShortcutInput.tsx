...
export const ShortcutInput: React.FC<ShortcutInputProps> = ({ label, value, onChange, reservedKeys = RESERVED_KEYS }) => {
  ...
  return (
    <div className="flex items-center justify-between gap-md p-md rounded-premium-sm bg-primary/[0.02] border border-border/10">
      ...
      <div className="relative w-3xl">
        <Input
          ...
          readOnly
          className="text-center font-bold text-primary h-xl rounded-lg border-primary/20 bg-background focus:ring-1 focus:ring-primary/20"
        />
      </div>
    </div>
  );
