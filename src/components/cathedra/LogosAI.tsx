        ...
        {isOpen && (
          ...
          >
            <CathedraCard padding="none" className="bg-card/40 backdrop-blur-3xl border-primary/[0.05] p-lg md:p-2xl lg:p-3xl space-y-md md:space-y-xl relative overflow-hidden shadow-premium rounded-[3rem]">
                ...
                {history.length > 0 && (
                  <>
                    <CathedraButton 
                      ...
                      className="rounded-full text-primary/60 hover:text-primary transition-colors h-xl w-xl px-0"
                      title="Exportar histórico"
                      icon={<Icons.Download className="w-sm h-sm" />}
                    />
                    <CathedraButton 
                      variant="ghost" 
                      size="sm"
                      ...
                      icon={<Icons.RotateCcw className="w-sm h-sm" />}
                    />
                  </>
                )}
                <CathedraButton variant="ghost" size="sm" onClick={onClose} className="rounded-full text-primary/60 hover:text-primary transition-colors h-xl w-xl px-0" icon={<Icons.X className="w-sm h-sm" />} />
              </div>

              <div className="flex items-center justify-between mb-lg md:mb-xl opacity-30">
                <div className="flex items-center gap-sm md:gap-md">
                  <div className="w-xl h-xl rounded-full bg-primary/[0.02] flex items-center justify-center text-primary/40 border border-primary/[0.05]">
                    <Icons.Sparkles className="w-sm h-sm" strokeWidth={0.5} />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-2xs">
                  <div className={`w-2xs h-2xs rounded-full ${
              ...
              <div className="space-y-md md:space-y-xl max-w-3xl mx-auto">
                ...
                {history.slice(-visibleMessages).map((msg, i) => (
                  <motion.div 
                    ...
                    className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} gap-sm`}
                  >
                    <div className={`max-w-full text-sm md:text-lg leading-[1.7] md:leading-[1.8] tracking-wide ${
                      msg.role === 'user' 
                        ? 'text-primary/60 font-serif italic border-r border-primary/10 pr-md md:pr-lg text-right' 
                        : 'text-foreground/80 font-serif font-light'
                    }`}>
                      {msg.role === 'assistant' && (
                        <div className="flex items-center gap-xs mb-sm md:mb-md opacity-20">
                          <div className="w-md h-px bg-primary" />
                ...
                {(isLoading || isTyping) && (
                  <div className="flex justify-start">
                    <div className="flex gap-sm opacity-10 py-lg">
                      {[0, 1, 2].map((i) => (
                        <motion.div 
                          key={i}
                          animate={{ opacity: [0.2, 0.5, 0.2], y: [0, -2, 0] }}
                          transition={{ duration: 2, repeat: Infinity, delay: i * 0.4 }}
                          className="w-2xs h-2xs bg-primary rounded-full" 
                        />
                      ))}
                    </div>
                  </div>
              ...
              <div className="max-w-xl mx-auto pt-lg md:pt-xl border-t border-primary/5">
                <form onSubmit={handleQuery} className="relative group flex items-center justify-center">
                  <input
                    ...
                    className="w-full bg-transparent border-none text-sm md:text-lg focus:ring-0 outline-none text-center font-serif italic placeholder:text-muted-foreground/30 py-sm md:py-md transition-all text-primary"
                  />
                  <button 
                    type="submit" 
                    disabled={isLoading || isTyping || !query.trim() || settings.totalSilence}
                    className="absolute right-0 text-primary/60 hover:text-primary transition-all disabled:opacity-0 p-xs"
                  >
                    <Icons.ArrowRight className="w-md h-md stroke-[1]" />
                  </button>
                </form>
                <div className="flex flex-col items-center gap-xs mt-lg">
                  <div className="w-3xs h-3xs bg-primary/10 rounded-full" />
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="p-lg md:p-xl border-b border-border/5 flex items-center justify-between">
              <div className="flex items-center gap-md">
                <div className="w-xl h-xl rounded-premium bg-primary/[0.02] border border-border/10 flex items-center justify-center text-primary/60">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-muted-foreground/60 uppercase font-black tracking-widest mt-2xs">Mentor Espiritual</p>
                </div>
              </div>
                ...
                {history.length > 0 && (
                  <>
                    <CathedraButton 
                      ...
                      size="sm" 
                      onClick={exportHistory} 
                      className="rounded-full hover:bg-primary/[0.02] text-primary/60 hover:text-primary transition-colors h-xl w-xl px-0"
                      title="Exportar histórico"
                      icon={<Icons.Download className="w-md h-md" />}
                    />
                    <CathedraButton 
                      variant="ghost" 
                      size="sm" 
                      ...
                      className="rounded-full hover:bg-primary/[0.02] text-primary/60 hover:text-primary transition-colors h-xl w-xl px-0"
                      title="Limpar histórico"
                      icon={<Icons.RotateCcw className="w-md h-md" />}
                    />
                  </>
                )}
                <CathedraButton variant="ghost" size="sm" onClick={onClose} className="rounded-full hover:bg-primary/[0.02] text-primary/60 hover:text-primary transition-colors h-xl w-xl px-0" icon={<Icons.X className="w-md h-md" />} />
              </div>
            </div>

            <div className="px-lg md:px-xl py-sm bg-primary/[0.02] border-b border-border/5 flex items-center justify-between">
              <div className="flex items-center gap-xs">
                <div className={`w-2xs h-2xs rounded-full ${
                  settings.totalSilence ? 'bg-red-400' : (history.length > 0 ? 'bg-secondary animate-pulse' : 'bg-primary/20')
                }`} />
            ...
            <div className="flex-1 overflow-y-auto p-lg md:p-xl space-y-xl md:space-y-xl scrollbar-hide">
              {history.length === 0 && !selectedText && (
                <div className="text-center py-3xl space-y-xl">
                  ...
                  <p className="text-lg text-muted-foreground/40 font-serif italic leading-relaxed max-w-[280px] mx-auto">
                    ...
                    <br/>
                    <span className="text-sm uppercase tracking-widest font-black mt-md block">
              ...
              {history.length > visibleMessages && (
                <div className="flex justify-center pb-xl">
                  <CathedraButton 
                    ...
                    size="sm" 
                    onClick={() => setVisibleMessages(prev => prev + 10)}
                    className="text-[9px] font-black uppercase tracking-widest text-primary/60 hover:text-primary h-auto py-xs"
                  >
              ...
              {history.slice(-visibleMessages).map((msg, i) => (
                <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} gap-sm animate-in fade-in slide-in-from-bottom-xs duration-500`}>
                  ...
                  <span className="text-[8px] font-bold uppercase tracking-widest text-primary/60 px-md">
            ...
            <div className="p-lg md:p-xl border-t border-border/5 bg-background/30">
              <form onSubmit={handleQuery} className="relative group">
                <input
                  ...
                  disabled={settings.totalSilence}
                  className="w-full bg-transparent border-b border-border/10 py-md px-0 text-base md:text-lg focus:border-primary/40 outline-none transition-all placeholder:text-muted-foreground/60 font-serif italic"
                />
                <button 
                  type="submit"
                  disabled={isLoading || isTyping || settings.totalSilence}
                  className="absolute right-0 top-2xs/2 -translate-y-1/2 w-xl h-xl rounded-full flex items-center justify-center text-primary hover:scale-110 active:scale-95 transition-all disabled:opacity-30"
                >
