...
const ProGate: React.FC<ProGateProps> = ({ isPremium, isLoggedIn, onLogin, children }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-6 page-enter">
      <div className="w-20 h-20 rounded-premium bg-primary/10 flex items-center justify-center">
      ...
      <div className="bg-card border border-border p-6 rounded-premium max-w-sm w-full space-y-4 shadow-sm">
        ...
        {!isLoggedIn ? (
          ...
          <Button
            ...
            className="w-full py-4 bg-primary text-primary-foreground rounded-full font-black uppercase text-premium-tiny tracking-widest shadow-premium-hover hover:opacity-90 transition-all"
          >
            Assinar PRO
          </Button>
        )}
