      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            ref={panelRef}
            className="fixed right-0 top-0 bottom-0 w-full max-w-sm bg-card border-l border-border/40 shadow-premium-hover z-[201] p-xl flex flex-col outline-none"
            tabIndex={-1}
            role="dialog"
          ...
          >
              ...
              <section className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4 pt-lg">
                    ...
                    <div className="flex bg-muted/30 rounded-xl p-2xs gap-2xs border border-primary/5">
                      {(['small', 'medium', 'large', 'extra-large'] as const).map((s) => (
                        <button
                          key={s}
                          onClick={() => updateSettings({ fontSize: s })}
                          className={`flex-1 py-xs text-[10px] font-bold rounded-lg transition-all ${
              ...
              <section className="space-y-6 pt-md">
                ...
                <div className="px-xs">
                  <Slider 
                    ...
                    className="py-md"
                  />
                  <p className="text-[10px] text-muted-foreground leading-relaxed italic mt-xs">
                  ...
                  <div className="flex flex-col gap-xs">
                    {(['always', 'first_selection', 'never'] as const).map((mode) => (
                      <button
                        ...
                        className={`flex items-center justify-between p-md rounded-premium border transition-all ${
                      ...
                      >
                        ...
                        {settings.logosSuggestions === mode && <Icons.Check className="w-md h-md" />}
                      </button>
                    ))}
                  </div>
                  <p className="text-[10px] text-muted-foreground leading-relaxed italic px-2xs">
              ...
              <section className="space-y-6 pt-xl border-t border-border/10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-sm">
                    <Button 
                      variant="ghost" 
                      className="h-2xl rounded-xl text-[9px] font-bold uppercase tracking-[0.2em] text-primary/60 border border-primary/5 hover:border-primary/10"
                      onClick={() => handleExport('json')}
                    >
                      <Icons.Database className="w-sm h-sm mr-xs" />
                      JSON
                    </Button>
                    <Button 
                      variant="ghost" 
                    ...
                    >
                      <Icons.FileText className="w-sm h-sm mr-xs" />
                      PDF
                    </Button>
                  </div>
                        ...
                        {auditResult.contrastIssues.slice(0, 5).map((issue, idx) => (
                          <li key={idx} className="text-[9px] text-amber-600/70 leading-relaxed italic border-b border-amber-500/5 pb-xs last:border-0">
                            <div className="flex justify-between items-start gap-xs">
                              ...
                              <span className="font-black text-[8px] bg-amber-500/10 px-2xs rounded">Ratio: {issue.ratio}</span>
                            </div>
                            <div className="text-[8px] text-amber-700/60 mt-3xs">
                              {issue.suggestion}
                            </div>
                            <a href={`/design-system?search=contrast`} className="text-[7px] uppercase tracking-tighter underline text-amber-700/40 hover:text-amber-700 block mt-2xs">Ver Token</a>
                          </li>
                        ))}
                        {auditResult.contrastIssues.length > 5 && (
                          <li className="text-[8px] text-amber-600/40 uppercase font-bold tracking-widest pt-xs text-center">
                            + {auditResult.contrastIssues.length - 5} problemas adicionais (veja relatório completo)
                          </li>
                        )}
                      </ul>
                      <p className="text-[8px] text-primary/40 leading-relaxed pt-xs">
            ...
            <div className="pt-xl border-t border-border/10">
              <Button className="w-full rounded-full h-2xl text-[10px] font-bold uppercase tracking-[0.3em] shadow-premium btn-premium-primary" onClick={onClose}>
                Concluído
              </Button>
            </div>
          </motion.div>
