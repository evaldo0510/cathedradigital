...
const SaintDetail: React.FC<{ saint: Saint; onClose: () => void; autoReflect?: boolean }> = ({ saint, onClose, autoReflect = false }) => {
  ...
  >
    ...
    >
      ...
      {/* Image Sidebar */}
      <div className="w-full md:w-xs/5 h-4xl md:h-auto relative overflow-hidden flex-shrink-0">
        <SacredImage src={saint.image} className="w-full h-full object-cover" alt={saint.name} />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
        
        <div className="absolute bottom-xl left-xl right-xl text-white space-y-xs">
          <span className="text-xs font-black uppercase tracking-widest bg-primary px-sm py-2xs rounded-full mb-md inline-block">
      ...
      {/* Content Area */}
      <div className="flex-1 p-lg md:p-2xl overflow-y-auto no-scrollbar space-y-xl">
          ...
          <div className="flex-1 flex justify-end items-center gap-xs">
            {(saint as any).url && (
              <Button 
                ...
                variant="outline"
                className="bg-foreground/5 hover:bg-foreground/10 text-foreground border-border/20 text-xs font-black uppercase tracking-widest h-xl px-md rounded-full flex items-center gap-xs transition-all"
              >
        ...
        <section className="space-y-md">
          ...
          <div className="prose prose-sm dark:prose-invert max-w-none">
            ...
            {saint.fullBio && (
              <div className="mt-lg text-muted-foreground leading-relaxed text-sm space-y-md">
                {saint.fullBio.split('\n\n').map((paragraph, pIdx) => (
                  <p key={pIdx}>
        ...
        <motion.div 
          ...
          className="bg-primary/5 rounded-[2rem] p-lg md:p-xl border border-primary/10 flex flex-col md:flex-row items-center justify-between gap-lg group hover:bg-primary/10 transition-all shadow-md"
        >
          <div className="flex items-center gap-md">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-primary/60 mb-2xs">Transformar Inspiração em Prática</p>
          ...
          <Button 
            ...
            className="h-2xl px-xl bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase text-xs tracking-[0.2em] rounded-full shadow-premium shadow-primary/20 group/btn transition-all"
          >
