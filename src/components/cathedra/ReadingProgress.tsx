  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[150] pointer-events-none pb-[env(safe-area-inset-bottom)]">
        <div className="max-w-[var(--layout-max-width)] mx-auto px-lg md:px-3xl py-lg flex flex-col gap-md items-end">
          ...
          <AnimatePresence>
            {isVisible && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 20 }}
                className="pointer-events-auto flex items-center gap-sm"
              >
                {/* Bookmark Button */}
                <Button
                  ...
                  variant="outline"
                  size="icon"
                  className="w-2xl h-2xl rounded-full shadow-premium bg-background/80 backdrop-blur-md border-primary/10 hover:border-primary/30 tap-premium"
                  aria-label="Marcar posição atual"
                >
                  <Bookmark className="w-md h-md text-primary" />
                </Button>
                ...
                {(savedParagraphId || localStorage.getItem('cathedra_last_paragraph')) && (
                  ...
                  >
                    <CornerRightUp className="w-md h-md text-primary" />
                  </Button>
                )}
                ...
                {showResume && onResumeLast && (
                  <Button
                    ...
                    variant="secondary"
                    size="sm"
                    className="rounded-full shadow-premium text-[10px] font-black uppercase tracking-widest px-xl h-2xl border-2 border-secondary bg-secondary text-primary hover:bg-secondary/90 transition-all group relative overflow-hidden tap-premium"
                  >
                    <motion.span 
                      animate={{ scale: [1, 1.05, 1] }} 
                      transition={{ duration: 2, repeat: Infinity }}
                      className="flex items-center gap-xs relative z-10"
                    >
                      <Icons.History className="w-md h-md group-hover:rotate-[-45deg] transition-transform" />
                      Retomar
                    </motion.span>
          ...
          {!isSubtle && (
            <div className="w-full pointer-events-auto bg-background/40 backdrop-blur-xl border border-primary/5 rounded-full p-2xs shadow-premium overflow-hidden">
              <div className="flex items-center justify-between px-md mb-2xs">
              ...
              <Progress 
                value={progress} 
                className="h-2xs bg-primary/5 cursor-pointer" 
                onClick={(e) => {
